...
#pragma clang diagnostic pop
    ...
    id<MTLBuffer> maxResultsBuf =
        [device newBufferWithBytes:&max_results
                            length:sizeof(uint32_t)
                           options:MTLResourceStorageModeShared];
    ...
    NSUInteger threadGroupSize =
        std::min((NSUInteger)num_probe, pipelineState.maxTotalThreadsPerThreadgroup);
    MTLSize threadsPerGroup =
        MTLSizeMake(threadGroupSize > 0 ? threadGroupSize : 1, 1, 1);
    ...
    std::cout << "CPU Ground Truth: found " << cpu_pairs.size() << " matches in "
              << cpu_ms << " ms\n";

    // 2. Run Metal GPU Implementation
    id<MTLBuffer> gpu_build_buf =
        [device newBufferWithBytes:large_build.data()
                            length:sizeof(BoundingBox) * N_BUILD
                           options:MTLResourceStorageModeShared];
    id<MTLBuffer> gpu_probe_buf =
        [device newBufferWithBytes:large_probe.data()
                            length:sizeof(BoundingBox) * N_PROBE
                           options:MTLResourceStorageModeShared];
    id<MTLBuffer> gpu_out_buf =
        [device newBufferWithLength:sizeof(MatchPair) * MAX_RESULTS
                            options:MTLResourceStorageModeShared];
    uint32_t zero = 0;
    id<MTLBuffer> gpu_count_buf =
        [device newBufferWithBytes:&zero
                            length:sizeof(uint32_t)
                           options:MTLResourceStorageModeShared];
    uint32_t build_count_param = N_BUILD;
    id<MTLBuffer> gpu_nbuild_buf =
        [device newBufferWithBytes:&build_count_param
                            length:sizeof(uint32_t)
                           options:MTLResourceStorageModeShared];
    ...
    NSUInteger diffThreadsPerGroup =
        std::min((NSUInteger)N_PROBE, pipelineState.maxTotalThreadsPerThreadgroup);
    [diffEnc dispatchThreads:diffGridSize
        threadsPerThreadgroup:MTLSizeMake(diffThreadsPerGroup, 1, 1)];
    [diffEnc endEncoding];
    ...
    std::cout << "Metal GPU:        found " << gpu_hits << " matches in " << gpu_ms
              << " ms (Kernel + Sync)\n";
    ...
    std::cout << "\n>>> DIFFERENTIAL VALIDATION PASSED: 100% exact parity between CPU "
                 "and Metal GPU! <<<\n";
  }
  return 0;
}
