...
#pragma clang diagnostic pop
      ...
      id<MTLLibrary> hashLib = [device newLibraryWithSource:hashSrc
                                                    options:opts
                                                      error:&err];
      if (!hashLib) {
        set_last_error(std::string("Error compiling Spatial Hash shaders: ") +
        ...
        if (!pso_grid_count || !pso_grid_populate || !pso_grid_probe) {
          set_last_error(std::string("Error creating Spatial Hash PSOs: ") +
      ...
      if (has_hw_rt) {
        ...
        id<MTLLibrary> rtLib = [device newLibraryWithSource:rtSrc
                                                    options:opts
                                                      error:&err];
        if (!rtLib) {
          has_hw_rt = false;
          ...
          pso_rt_probe = [device newComputePipelineStateWithFunction:fnRtProbe
                                                               error:&err];
          if (!pso_rt_probe) {
            has_hw_rt = false;
          }
  ...
  bool build_rt_index() {
    @autoreleasepool {
      ...
      // Expand conservative bounds to prevent grazing-ray precision misses on boundaries
      // across any coordinate scale
      for (uint32_t i = 0; i < n; ++i) {
        const auto& b = build_boxes[i];
        if (!is_valid_box(b)) {
          // Park NaN/Inf/inverted rows at a dummy bounding box outside ray range (z in
          // [10, 11])
          mtl_boxes[i].min = MTLPackedFloat3Make(0.0f, 0.0f, 10.0f);
          mtl_boxes[i].max = MTLPackedFloat3Make(0.0f, 0.0f, 11.0f);
          continue;
        }
        // Conservative expansion to prevent grazing ray misses across any coordinate
        // scale, avoiding denormal flush-to-zero (FTZ) around zero boundaries on Apple
        // Silicon RT units.
        float scale_x =
            std::max(std::max(std::abs(b.xmin), std::abs(b.xmax)), b.xmax - b.xmin);
        float scale_y =
            std::max(std::max(std::abs(b.ymin), std::abs(b.ymax)), b.ymax - b.ymin);
      ...
      [enc buildAccelerationStructure:rt_accel
      ...
      if ([cmd status] != MTLCommandBufferStatusCompleted) {
        NSString* errStr =
            [cmd.error localizedDescription] ?: @"Error during RT BVH build";
    ...
    @autoreleasepool {
      ...
      uint32_t dims[4] = {
          ...
          static_cast<uint32_t>(std::clamp(
              static_cast<float>(std::sqrt(valid_count) * 1.5f), 64.0f, 256.0f))};
      ...
      id<MTLBuffer> buf_counts =
          [device newBufferWithLength:sizeof(uint32_t) * total_cells
                              options:MTLResourceStorageModeShared];
      if (!buf_counts) {
      ...
      id<MTLBuffer> buf_params =
          [device newBufferWithBytes:&hier_params
      ...
      NSUInteger tg_count =
          std::min((NSUInteger)256, pso_grid_count.maxTotalThreadsPerThreadgroup);
      [enc1 dispatchThreads:MTLSizeMake(n, 1, 1)
          threadsPerThreadgroup:MTLSizeMake(tg_count, 1, 1)];
      [enc1 endEncoding];
      [cmd1 commit];
      [cmd1 waitUntilCompleted];
      if ([cmd1 status] != MTLCommandBufferStatusCompleted) {
        NSString* errStr =
            [cmd1.error localizedDescription] ?: @"Error during count pass";
        set_last_error([errStr UTF8String]);
        return false;
      }
      ...
      buf_grid_offsets = [device newBufferWithBytes:offsets.data()
                                            ...
                                            options:MTLResourceStorageModeShared];
      buf_grid_entries =
          [device newBufferWithLength:sizeof(uint32_t) * std::max(1u, total_entries)
                              options:MTLResourceStorageModeShared];
      if (!buf_grid_offsets || !buf_grid_entries) {
      ...
      NSUInteger tg_pop =
          std::min((NSUInteger)256, pso_grid_populate.maxTotalThreadsPerThreadgroup);
      [enc2 dispatchThreads:MTLSizeMake(n, 1, 1)
          threadsPerThreadgroup:MTLSizeMake(tg_pop, 1, 1)];
      [enc2 endEncoding];
      [cmd2 commit];
      [cmd2 waitUntilCompleted];
      if ([cmd2 status] != MTLCommandBufferStatusCompleted) {
        NSString* errStr =
            [cmd2.error localizedDescription] ?: @"Error during populate pass";
        set_last_error([errStr UTF8String]);
        return false;
      }
      return true;
...
void MetalSpatialIndex::set_index_type(IndexType type) { impl_->requested_type = type; }

IndexType MetalSpatialIndex::get_index_type() const { return impl_->requested_type; }

IndexType MetalSpatialIndex::get_active_index_type() const {
  return impl_->active_type.load();
}

bool MetalSpatialIndex::supports_hardware_rt() const { return impl_->has_hw_rt; }
...
bool MetalSpatialIndex::finish_building() {
  ...
  impl_->build_count = n;
  impl_->buf_build_boxes =
      [impl_->device newBufferWithBytes:impl_->build_boxes.data()
                                 length:sizeof(BoundingBox) * n
                                options:MTLResourceStorageModeShared];
  ...
  if (impl_->has_hw_rt && (impl_->requested_type == IndexType::HardwareRT ||
                           impl_->requested_type == IndexType::Auto)) {
...
bool MetalSpatialIndex::probe(const float* rects_flat, uint32_t count,
  ...
  @autoreleasepool {
    ...
    for (uint32_t i = 0; i < count; ++i) {
      ...
      if (probe_boxes[i].xmin != probe_boxes[i].xmax ||
          probe_boxes[i].ymin != probe_boxes[i].ymax) {
        is_points = false;
        break;
      }
    }
    ...
    impl_->active_type.store(use_rt ? IndexType::HardwareRT : IndexType::SpatialHash,
                             std::memory_order_relaxed);

    auto t0 = std::chrono::high_resolution_clock::now();

    id<MTLBuffer> buf_probe =
        [impl_->device newBufferWithBytes:rects_flat
    ...
    uint64_t initial_max = std::min(
        max_pairs_capacity, std::max(static_cast<uint64_t>(count) * 8ULL, 1000000ULL));
    ...
    id<MTLBuffer> buf_out =
        [impl_->device newBufferWithLength:sizeof(MatchPair) * max_results
                                   options:MTLResourceStorageModeShared];
    id<MTLBuffer> buf_count =
    ...
    *((uint32_t*)[buf_count contents]) = 0;
    ...
    auto dispatch_query = [&](uint32_t curr_max) -> bool {
      for (uint32_t probe_offset = 0; probe_offset < count; probe_offset += CHUNK_SIZE) {
        ...
        if (use_rt) {
          id<MTLBuffer> buf_num_probes =
              [impl_->device newBufferWithBytes:&chunk_count
                                         length:sizeof(uint32_t)
                                        options:MTLResourceStorageModeShared];
          id<MTLBuffer> buf_max_results =
              [impl_->device newBufferWithBytes:&curr_max
                                         length:sizeof(uint32_t)
                                        options:MTLResourceStorageModeShared];
          id<MTLBuffer> buf_probe_offset =
              [impl_->device newBufferWithBytes:&probe_offset
                                         length:sizeof(uint32_t)
                                        options:MTLResourceStorageModeShared];
          ...
          NSUInteger tg_rt = std::min((NSUInteger)256,
                                      impl_->pso_rt_probe.maxTotalThreadsPerThreadgroup);
          [enc dispatchThreads:MTLSizeMake(chunk_count, 1, 1)
              threadsPerThreadgroup:MTLSizeMake(tg_rt, 1, 1)];
          [enc endEncoding];
          [cmd commit];
          [cmd waitUntilCompleted];
          if ([cmd status] != MTLCommandBufferStatusCompleted) {
            NSString* errStr =
                [cmd.error localizedDescription] ?: @"Error during RT probe execution";
          ...
          id<MTLBuffer> buf_params =
              [impl_->device newBufferWithBytes:&params
                                         length:sizeof(HierarchicalGridParamsInternal)
                                        options:MTLResourceStorageModeShared];
          if (!buf_params) {
          ...
          NSUInteger tg_grid = std::min(
              (NSUInteger)256, impl_->pso_grid_probe.maxTotalThreadsPerThreadgroup);
          [enc dispatchThreads:MTLSizeMake(chunk_count, 1, 1)
              threadsPerThreadgroup:MTLSizeMake(tg_grid, 1, 1)];
          [enc endEncoding];
          [cmd commit];
          [cmd waitUntilCompleted];
          if ([cmd status] != MTLCommandBufferStatusCompleted) {
            NSString* errStr =
                [cmd.error localizedDescription] ?: @"Error during grid probe execution";
            impl_->set_last_error([errStr UTF8String]);
            return false;
          }
        }
    ...
    auto t1 = std::chrono::high_resolution_clock::now();
    impl_->last_probe_time_ms.store(
...
uint32_t MetalSpatialIndex::get_build_count() const { return impl_->build_count; }
