...
int main() {
    @autoreleasepool {
        std::cout << "\n======================================================================================\n";
        std::cout << " [TEMPORARY / TO BE REMOVED] SedonaDB Metal vs. CPU Heavy Stress Benchmark\n";
        std::cout << " Measuring CPU execution (target: 5-10 seconds) vs. Apple Silicon GPU (Metal)\n";
        std::cout << "======================================================================================\n\n";
        ...
        // ====================================================================================
        // BENCHMARK 1: High-Density Spatial Join Filter (100,000 x 100,000 = 10 Billion Checks)
        // ====================================================================================
        std::cout << "--------------------------------------------------------------------------------------\n";
        std::cout << " BENCHMARK 1: Spatial Join Candidate Filtering (10 Billion Pairwise Combinations)\n";
        ...
        // 150,000 probes against 100,000 build boxes = 15.0 Billion intersection checks (~6.5 - 7.0 s on CPU)
        ...
        std::cout << "Running CPU Pairwise Baseline (15.0 Billion intersection tests)..." << std::flush;
        ...
        double cpu_join_sec = std::chrono::duration<double>(cpu_join_end - cpu_join_start).count();
        std::cout << " DONE in " << std::fixed << std::setprecision(3) << cpu_join_sec << " s\n";
        std::cout << "  -> CPU Result: " << cpu_join_matches << " matches in "
                  << std::setprecision(2) << cpu_join_sec << " s (" << (cpu_join_sec * 1000.0) << " ms)\n";

        // 2. GPU MetalSpatialIndex Acceleration (Spatial Hash / RT)
        std::cout << "Running Apple Silicon GPU (MetalSpatialIndex across full 100k x 150k)..." << std::flush;
        ...
        double gpu_join_sec = std::chrono::duration<double>(gpu_join_end - gpu_join_start).count();
        std::cout << " DONE in " << std::fixed << std::setprecision(4) << gpu_join_sec << " s\n";
        ...
        assert(out_build.size() == cpu_join_matches && "Match count mismatch between CPU and GPU!");
        std::cout << "  >>> Parity: 100% EXACT MATCH (" << out_build.size() << " pairs) <<<\n";
        ...
        // ====================================================================================
        std::cout << "--------------------------------------------------------------------------------------\n";
        std::cout << " BENCHMARK 2: Exact Point-in-Polygon (MSL Winding Number with Holes vs. CPU)\n";
        std::cout << " Workload: 100 complex non-convex multi-ring polygons x 50,000 probe points\n";
        std::cout << " Total: 5,000,000 polygon evaluations (approx 150 Million ray-edge intersections)\n";
        std::cout << "--------------------------------------------------------------------------------------\n";
        ...
        for (uint32_t p = 0; p < NUM_POLYS; ++p) {
            ...
            if (p % 2 == 0) {
                // Donut with hole
                uint32_t outer = add_ring({
                    {ox, oy}, {ox + 80.0f, oy}, {ox + 80.0f, oy + 80.0f}, {ox, oy + 80.0f}, {ox, oy}
                });
                add_ring({
                    {ox + 25.0f, oy + 25.0f}, {ox + 55.0f, oy + 25.0f},
                    {ox + 55.0f, oy + 55.0f}, {ox + 25.0f, oy + 55.0f}, {ox + 25.0f, oy + 25.0f}
                });
                polygons.push_back(PolygonGeom{outer, 1});
            } else {
                // Non-convex L-shape
                uint32_t outer = add_ring({
                    {ox, oy}, {ox + 90.0f, oy}, {ox + 90.0f, oy + 30.0f},
                    {ox + 30.0f, oy + 30.0f}, {ox + 30.0f, oy + 90.0f}, {ox, oy + 90.0f}, {ox, oy}
                });
        ...
        // Build candidate list: all 50,000 points checked against all 100 polygons = 5,000,000 pairs
        ...
        std::cout << "Running CPU Reference PIP (" << TOTAL_PAIRS << " candidate evaluations)..." << std::flush;
        ...
        for (uint32_t i = 0; i < TOTAL_PAIRS; ++i) {
            PointLocation loc = cpu_locate_point_in_polygon(
                test_pts[pip_candidates[i].point_idx],
                polygons[pip_candidates[i].polygon_idx],
                rings,
                vertices
            );
        ...
        double cpu_pip_sec = std::chrono::duration<double>(cpu_pip_end - cpu_pip_start).count();
        std::cout << " DONE in " << std::fixed << std::setprecision(3) << cpu_pip_sec << " s\n";
        std::cout << "  -> CPU Result: " << cpu_pip_matches << " exact matches in "
                  << std::setprecision(2) << cpu_pip_sec << " s (" << (cpu_pip_sec * 1000.0) << " ms)\n";
        ...
        id<MTLBuffer> buf_cands = [device newBufferWithBytes:pip_candidates.data() length:sizeof(CandidatePair) * TOTAL_PAIRS options:MTLResourceStorageModeShared];
        id<MTLBuffer> buf_polys = [device newBufferWithBytes:polygons.data() length:sizeof(PolygonGeom) * polygons.size() options:MTLResourceStorageModeShared];
        id<MTLBuffer> buf_rings = [device newBufferWithBytes:rings.data() length:sizeof(PolygonRing) * rings.size() options:MTLResourceStorageModeShared];
        id<MTLBuffer> buf_verts = [device newBufferWithBytes:vertices.data() length:sizeof(Point2D) * vertices.size() options:MTLResourceStorageModeShared];
        id<MTLBuffer> buf_points = [device newBufferWithBytes:test_pts.data() length:sizeof(Point2D) * test_pts.size() options:MTLResourceStorageModeShared];
        id<MTLBuffer> buf_out = [device newBufferWithLength:sizeof(CandidatePair) * (cpu_pip_matches + 1000) options:MTLResourceStorageModeShared];
        uint32_t zero = 0;
        id<MTLBuffer> buf_count = [device newBufferWithBytes:&zero length:sizeof(uint32_t) options:MTLResourceStorageModeShared];
        uint32_t n_cands = TOTAL_PAIRS;
        id<MTLBuffer> buf_ncands = [device newBufferWithBytes:&n_cands length:sizeof(uint32_t) options:MTLResourceStorageModeShared];
        uint32_t max_res = static_cast<uint32_t>(cpu_pip_matches + 1000);
        id<MTLBuffer> buf_max = [device newBufferWithBytes:&max_res length:sizeof(uint32_t) options:MTLResourceStorageModeShared];
        ...
        NSUInteger tg = std::min((NSUInteger)TOTAL_PAIRS, pipeline.maxTotalThreadsPerThreadgroup);
        ...
        double gpu_pip_sec = std::chrono::duration<double>(gpu_pip_end - gpu_pip_start).count();
        uint32_t gpu_pip_matches = *(uint32_t*)[buf_count contents];
        std::cout << " DONE in " << std::fixed << std::setprecision(4) << gpu_pip_sec << " s\n";
        ...
        std::cout << "======================================================================================\n";
        std::cout << " SUMMARY:\n";
        std::cout << "   1. Spatial Join (100k x 100k): CPU ~" << std::setprecision(1) << cpu_join_sec
                  << " s  vs.  GPU " << std::setprecision(1) << (gpu_join_sec * 1000.0) << " ms  ("
                  << std::setprecision(0) << join_speedup << "x speedup)\n";
        std::cout << "   2. Point-in-Polygon (5M pairs): CPU ~" << std::setprecision(1) << cpu_pip_sec
                  << " s  vs.  GPU " << std::setprecision(1) << (gpu_pip_sec * 1000.0) << " ms  ("
                  << std::setprecision(0) << pip_speedup << "x speedup)\n";
        std::cout << "======================================================================================\n";
    }
    return 0;
}
