...
use sedona_libgpuspatial::{
    GpuSpatialIndex as RawGpuSpatialIndex, GpuSpatialOptions, GpuSpatialRefiner as RawGpuSpatialRefiner,
...
impl PlatformSpatialIndex {
    ...
    pub fn push_build(&mut self, rects: &[[f32; 4]]) -> Result<()> {
        self.raw
            .push_build(rects)
            .map_err(|e| DataFusionError::Execution(format!("Failed to push rectangles to GPU spatial index: {e:?}")))
    }

    pub fn finish_building(&mut self) -> Result<()> {
        self.raw
            .finish_building()
            .map_err(|e| DataFusionError::Execution(format!("Failed to build spatial index on GPU: {e:?}")))
    }

    pub fn probe(&self, rects: &[[f32; 4]]) -> Result<(Vec<u32>, Vec<u32>)> {
        self.raw
            .probe(rects)
            .map_err(|e| DataFusionError::Execution(format!("GPU spatial query failed: {e:?}")))
    }
}
...
impl PlatformSpatialRefiner {
    ...
    pub fn init_build_schema(&mut self, data_type: &DataType) -> Result<()> {
        self.raw
            .init_build_schema(data_type)
            .map_err(|e| DataFusionError::Execution(format!("Failed to init schema for refiner: {e:?}")))
    }

    pub fn push_build(&mut self, array: &ArrayRef) -> Result<()> {
        self.raw
            .push_build(array)
            .map_err(|e| DataFusionError::Execution(format!("Failed to add geometries to GPU refiner: {e:?}")))
    }

    pub fn finish_building(&mut self) -> Result<()> {
        self.raw
            .finish_building()
            .map_err(|e| DataFusionError::Execution(format!("Failed to build spatial refiner on GPU: {e:?}")))
    }
